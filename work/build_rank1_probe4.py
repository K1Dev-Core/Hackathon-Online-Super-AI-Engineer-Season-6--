"""Extend the validated PUBLISH-132 submission with its 137-byte sibling."""
...
BASE_PATH = ROOT / "outputs" / "submission_rank1_probe3_publish132.csv"
TEST_PATH = ROOT / "data" / "X_test.csv"
OUTPUT_PATH = ROOT / "outputs" / "submission_rank1_probe4_publish137.csv"
...
def main() -> None:
    ...
    candidates = (
        test["frame.len"].eq(194)
        & test["tcp.len"].eq(140)
        & test["tcp.window_size"].eq(256)
        & test["mqtt.msgtype"].eq(3)
        & test["mqtt.topic_len"].eq(30)
        & test["mqtt.len"].eq(137)
        & test["tcp.stream"].eq(1)
    )
    assert int(candidates.sum()) == 6
